    ...
    categoryGrid.innerHTML = GARMENTS_DATA.shopCategories.map((cat, idx) => `
      <div class="shop-cat-card garment-sharp group bg-white border border-stone-200 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between overflow-hidden">
        <div>
          ...
          <a href="${cat.anchor}" ${cat.target ? `target="${cat.target}" rel="noopener noreferrer"` : ''} class="garment-img-frame aspect-square w-full border-b border-stone-200 relative block">
          ...
          <div class="p-5 sm:p-6">
            <a href="${cat.anchor}" ${cat.target ? `target="${cat.target}" rel="noopener noreferrer"` : ''} class="block">
        ...
        <div class="p-5 sm:p-6 pt-0 mt-auto">
          <div class="pt-4 border-t border-stone-150 flex items-center justify-between">
            <a href="${cat.anchor}" ${cat.target ? `target="${cat.target}" rel="noopener noreferrer"` : ''} class="text-xs font-heading font-bold uppercase tracking-widest text-navy group-hover:text-gold transition-colors flex items-center gap-1.5">
              <span>${cat.actionText || 'Enquire Category'}</span>
